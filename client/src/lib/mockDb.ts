...
const defaultCharities: Charity[] = [
  {
    ...
    name: "WaterAid",
    description:
      "Bringing clean water, decent toilets and good hygiene to communities worldwide.",
    category: "Clean water",
    accent: "aqua",
    featured: true,
    impact: "38% of members",
  },
  {
    id: "c2",
    name: "Mind UK",
    ...
    accent: "violet",
    featured: true,
    impact: "29% of members",
  },
  {
    id: "c3",
    name: "StreetVet",
    description:
      "Delivering free essential veterinary care to pets of people experiencing homelessness.",
    category: "Animal welfare",
    accent: "coral",
    featured: false,
    impact: "18% of members",
  },
  {
    id: "c4",
    name: "Trees for Cities",
    description:
      "Working to plant trees and green up cities worldwide to fight the climate crisis.",
    category: "Climate",
    accent: "lime",
    featured: false,
    impact: "15% of members",
  },
];
...
class MockDB {
  ...
  init() {
    ...
    if (!localStorage.getItem("dh_charities"))
      this.set("charities", defaultCharities);
  ...
  addScore(
    ...
    return {
      success: true,
      message: "Score added successfully. Rolling 5 updated.",
    };
  }

  // Charities
  getCharities(): Charity[] {
    return this.get("charities", defaultCharities);
  }
  selectCharity(userId: string, charityId: string) {
    this.updateUser(userId, { charityId });
  }
  ...
  simulateDraw(mode: "random" | "weighted" = "weighted") {
    ...
    users
      ...
      .forEach(u => {
        ...
        const matchCount = uScores.filter(s =>
    ...
    if (!jackpotWon) {
      // Simulate saving rollover state
      this.set("jackpotRollover", (this.get("jackpotRollover", 0) as number) + tierPools[5]);
    } else {
      this.set("jackpotRollover", 0);
    }
